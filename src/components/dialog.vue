<template>
  <div class="zr-dialog" :class="{ 'zr-dialog--show': show === true }">
    <div class="zr-dialog--mask"></div>
    <div class="zr-dialog--container">
      <div
        v-if="title"
        class="zr-dialog__header">{{ title }}</div>
      <div
        class="zr-dialog__content" :class="{ 'zr-dialog__content--title' : title }">{{ content }}</div>
      <div
        class="zr-dialog__footer" :class="computedClassStr">
        <block v-for="(item, index) in buttons" :key="item.text + '-' + item.type">
          <button
            class="zr-dialog__button zr-btn"
            :data-type="item.type"
            @click="_handleZrDialogButtonClick"
  ...
  export default {
    ...
    computed: {
      computedClassStr: function () {
        return this.buttonsShowVertical ? 'zr-dialog__footer--vertical' : 'zr-dialog__footer--horizon'
      },
